...
export default function ColdChainSection({ dispatchHub }) {
  return (
    <section className="rounded-[22px] bg-[#ffffff] border border-[#c3cda7] overflow-hidden shadow-sm">
      <div className="grid grid-cols-1 lg:grid-cols-12 items-stretch">
        ...
        <div className="lg:col-span-5 relative min-h-[220px]">
          <img
            src={dispatchHub.imageUrl}
            alt="Mandi Logistics Loading Bay"
          ...
          <div>
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <span className="text-[10px] font-mono uppercase tracking-widest text-[#1b6e53] font-bold bg-[#e6ecd5] px-2.5 py-0.5 rounded-full border border-[#c3cda7]">
                LOGISTICS & COLD-CHAIN TELEMETRY
              </span>
              <span className="text-xs font-mono text-[#6d6d6d]">Bay Gate #02 Active</span>
            </div>
            <h3 className="font-editorial text-2xl font-normal text-[#212529] mt-2">
              {dispatchHub.title}
            </h3>
            <p className="text-xs text-[#6d6d6d] mt-1 leading-relaxed">
              {dispatchHub.description}
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-[#c3cda7]/50 text-xs">
            {dispatchHub.metrics.map((m) => (
              <div key={m.label} className={`p-2.5 rounded-[14px] ${m.bg} border border-[#c3cda7]/60`}>
                <span className="text-[10px] text-[#6d6d6d] uppercase font-mono block">
                  {m.label}
                </span>
                <span className={`font-semibold ${m.mainColor} block mt-0.5`}>
                  {m.main}
                </span>
                <span className={`text-[10px] ${m.subColor || 'text-[#353535]'}`}>
                  {m.sub}
                </span>
              </div>
            ))}
